...
import com.logiflow.pedidoservice.util.JwtUtils;
    ...
    protected void doFilterInternal(
        ...
        if (authHeader != null && authHeader.startsWith("Bearer ")) {
            ...
            try {
                ...
                log.error(" PEDIDO-SERVICE - Error validando JWT: {}", e.getMessage());
                response.setStatus(HttpServletResponse.SC_UNAUTHORIZED);
                response.getWriter().write("{\"error\":\"Error de autenticación\"}");
                return;
            }
        } else {
            log.warn(" PEDIDO-SERVICE - Sin token Authorization");
            response.setStatus(HttpServletResponse.SC_UNAUTHORIZED);
